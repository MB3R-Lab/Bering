...
func TestDiscoverTopology_AppliesProvenanceAndOverlays(t *testing.T) {
	...
	doc := topology.Document{
		...
		Services: []topology.Service{
			{ID: "frontend", Name: "frontend", Replicas: intPtr(2)},
			{ID: "checkout", Name: "checkout", Replicas: intPtr(1)},
		},
		Edges: []topology.Edge{
			{ID: "frontend|checkout|sync|true", From: "frontend", To: "checkout", Kind: "sync", Blocking: boolPtr(true), Support: topology.Support{Observations: 1, Evidence: []string{"topology_api"}}},
		},
	...
	result, err := DiscoverTopology(doc, Options{
		...
		Overlays: []overlay.File{{
			...
			Services:  []overlay.ServiceOverlay{{ID: "checkout", Replicas: &replicas}},
